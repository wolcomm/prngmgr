...
class Command(BaseCommand):
    """Sync-sessions management command for prngmgr."""
    ...
    def handle(self, *args, **options):
        ...
        if options['router']:
            ...
            try:
                rtr = models.PeeringRouter.objects.get(
                    hostname=options['router'])
            except models.PeeringRouter.DoesNotExist:
                raise CommandError('PeeringRouter "%s" does not exist'
                                   % options['router'])
            rtrs.append(rtr)
        else:
        ...
        for rtr in rtrs:
            ...
            ifaces = models.PeeringRouterIXInterface.objects.filter(
                prngrtr=rtr)
            for iface in ifaces:
                ...
                peer_netixlans = models.NetworkIXLan.objects.filter(
                    ixlan=iface.netixlan.ixlan)
                for peer_netixlan in peer_netixlans:
                    ...
                    if iface.netixlan.ipaddr4 and peer_netixlan.ipaddr4:
                        ...
                        for entry in bgptable:
                            if (bgptable[entry]['cbgpPeer2Type']
                                    == prngsess.af
                                    and bgptable[entry]['cbgpPeer2RemoteAddr']
                                    == prngsess.peer_netixlan.ipaddr4
                                    and bgptable[entry]['cbgpPeer2RemoteAs']
                                    == prngsess.peer_netixlan.asn):
                    ...
                    if iface.netixlan.ipaddr6 and peer_netixlan.ipaddr6:
                        ...
                        for entry in bgptable:
                            if (bgptable[entry]['cbgpPeer2Type']
                                    == prngsess.af
                                    and bgptable[entry]['cbgpPeer2RemoteAddr']
                                    == prngsess.peer_netixlan.ipaddr6
                                    and bgptable[entry]['cbgpPeer2RemoteAs']
                                    == prngsess.peer_netixlan.asn):
                                # found a configured peering
                                count['bgpprng6'] += 1
            ...
            self.stdout.write("%d peering interfaces found"
                              % count['prngrtrifaces'])
            self.stdout.write("updated %d ipv4 peering sessions: \
                              %d provisioned in bgp"
                              % (count['prngsess4'], count['bgpprng4']))
            self.stdout.write("updated %d ipv6 peering sessions: \
                              %d provisioned in bgp"
                              % (count['prngsess6'], count['bgpprng6']))
            totals.update(count)

        # all done: print some stats
        self.stdout.write("finished updates for %d routers" % totals['rtrs'])
        self.stdout.write("%d peering interfaces found"
                          % totals['prngrtrifaces'])
        self.stdout.write("updated %d ipv4 peering sessions: \
                          %d provisioned in bgp"
                          % (totals['prngsess4'], totals['bgpprng4']))
        self.stdout.write("updated %d ipv6 peering sessions: \
                          %d provisioned in bgp"
                          % (totals['prngsess6'], totals['bgpprng6']))
    ...
    def _update_state(self, prngsess, bgpprng):
        ...
        if bgpprng:
            ...
            if prngsess.provisioning_state != \
                    models.PeeringSession.PROV_COMPLETE:
                prngsess.provisioning_state = \
                    models.PeeringSession.PROV_COMPLETE
                changed = True
            ...
            # check if the peering session was previously provisioned, and
            # reset if necessary
